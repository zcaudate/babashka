...
to catch and handle."
  ...
  (:require [babashka.impl.clojure.core.async.impl.ioc-macros :as ioc]
...
(defn ioc-alts! [state cont-block ports & {:as opts}]
  ...
  (when-let [cb (babashka.impl.clojure.core.async/do-alts
                  (fn [val]
...
(defn go
  ...
  (let [crossing-env (zipmap (keys &env) (repeatedly gensym))]
    `(let [c# (clojure.core.async/chan 1)
           captured-bindings# (clojure.core/get-thread-binding-frame-impl)]
       (dispatch/run
         (^:once fn* []
          (let [~@(mapcat (fn [[l sym]] [sym `(^:once fn* [] ~(vary-meta l dissoc :tag))]) crossing-env)
            ...
            (clojure.core.async.impl.ioc-macros/run-state-machine-wrapped state#))))
       c#)))
