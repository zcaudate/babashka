...
(defmacro aset-all!
  ...
  (let [bindings (partition 2 more)
    ...
    `(let [~arr-sym ~arr]
       ~@(map
          (fn [[idx val]]
            `(clojure.core.async.impl.ioc-macros/aset-object ~arr-sym ~idx ~val))
          bindings)
       ~arr-sym)))

(defmacro aset-all-internal! ;; called from within this namespace
  [arr & more]
  (assert (even? (count more)) "Must give an even number of args to aset-all!")
  (let [bindings (partition 2 more)
        arr-sym (gensym "statearr-")]
...
(defrecord Const [value]
  ...
  (emit-instruction [this state-sym]
    (if (= value ::value)
      `[~(:id this) (clojure.core.async.impl.ioc-macros/aget-object ~state-sym ~VALUE-IDX)]
      `[~(:id this) ~value])))
  ...
  (terminate-block [this state-sym _]
    `(do (case ~val-id
           ~@(concat (mapcat (fn [test blk]
                               `[~test (clojure.core.async.impl.ioc-macros/aset-all! ~state-sym
                                                  ~STATE-IDX ~blk)])
                             test-vals jmp-blocks)
                     (when default-block
                       `[(do (clojure.core.async.impl.ioc-macros/aset-all! ~state-sym ~STATE-IDX ~default-block)
                             :recur)])))
         :recur)))
...
(defrecord Jmp [value block]
  ...
  ITerminator
  (terminate-block [this state-sym _]
    `(do (clojure.core.async.impl.ioc-macros/aset-all! ~state-sym ~VALUE-IDX ~value ~STATE-IDX ~block)
         :recur)))
...
(defrecord Return [value]
  ...
  (terminate-block [this state-sym custom-terminators]
    (if-let [f (get custom-terminators (terminator-code this))]
      `(~f ~state-sym ~value)
      `(do (clojure.core.async.impl.ioc-macros/aset-all! ~state-sym
                      ~VALUE-IDX ~value
                      ~STATE-IDX ::finished)
           nil))))
...
(defrecord CondBr [test then-block else-block]
  ...
  (terminate-block [this state-sym _]
    `(do (if ~test
           (clojure.core.async.impl.ioc-macros/aset-all! ~state-sym
...
(defrecord PushTry [catch-block]
  ...
  IEmittableInstruction
  (emit-instruction [this state-sym]
    `[~'_ (clojure.core.async.impl.ioc-macros/aset-all! ~state-sym ~EXCEPTION-FRAMES (cons ~catch-block (clojure.core.async.impl.ioc-macros/aget-object ~state-sym ~EXCEPTION-FRAMES)))]))
...
(defrecord PopTry []
  ...
  IEmittableInstruction
  (emit-instruction [this state-sym]
    `[~'_ (clojure.core.async.impl.ioc-macros/aset-all! ~state-sym ~EXCEPTION-FRAMES (rest (clojure.core.async.impl.ioc-macros/aget-object ~state-sym ~EXCEPTION-FRAMES)))]))
...
(defrecord CatchHandler [catches]
  ...
  (terminate-block [this state-sym _]
    (let [ex (gensym 'ex)]
      `(let [~ex (clojure.core.async.impl.ioc-macros/aget-object ~state-sym ~VALUE-IDX)]
         (cond
           ~@(for [[handler-idx type] catches
                   i [`(instance? ~type ~ex) `(clojure.core.async.impl.ioc-macros/aset-all! ~state-sym ~STATE-IDX ~handler-idx)]]
               i)
           :else (throw ~ex))
         :recur))))
...
(defn- build-block-preamble [local-map idx state-sym blk]
  (let [args (->> (mapcat reads-from blk)
    ...
    (if (empty? args)
      []
      (mapcat (fn [sym]
                `[~sym (clojure.core.async.impl.ioc-macros/aget-object ~state-sym ~(id-for-inst local-map sym))])
              args))))
...
(defn- build-new-state [local-map idx state-sym blk]
  (let [results (->> blk
    ...
    (if-not (empty? results)
      [state-sym `(clojure.core.async.impl.ioc-macros/aset-all! ~state-sym ~@results)]
      [])))
...
(defn- emit-state-machine [machine num-user-params custom-terminators]
  (let [index (index-state-machine machine)
    ...
    `(fn state-machine#
       ([] (clojure.core.async.impl.ioc-macros/aset-all! (AtomicReferenceArray. ~state-arr-size)
                      ~FN-IDX state-machine#
                      ~STATE-IDX ~(:start-block machine)))
       ([~state-sym]
        (let [old-frame# (clojure.core/get-thread-binding-frame-impl)
              ret-value# (try
                           (clojure.core/reset-thread-binding-frame-impl (clojure.core.async.impl.ioc-macros/aget-object ~state-sym ~BINDINGS-IDX))
                           (loop []
                             (let [result# (case (int (clojure.core.async.impl.ioc-macros/aget-object ~state-sym ~STATE-IDX))
                                             ~@(mapcat
                                                (fn [[id blk]]
                               ...
                               (if (identical? result# :recur)
                                 ...
                                 result#)))
                           (catch Throwable ex#
                             (clojure.core.async.impl.ioc-macros/aset-all! ~state-sym ~VALUE-IDX ex#)
                             (if (seq (clojure.core.async.impl.ioc-macros/aget-object ~state-sym ~EXCEPTION-FRAMES))
                               (clojure.core.async.impl.ioc-macros/aset-all! ~state-sym ~STATE-IDX (first (clojure.core.async.impl.ioc-macros/aget-object ~state-sym ~EXCEPTION-FRAMES)))
                               (throw ex#))
                             :recur)
                           (finally
                             (clojure.core.async.impl.ioc-macros/aset-object ~state-sym ~BINDINGS-IDX (clojure.core/get-thread-binding-frame-impl))
                             (clojure.core/reset-thread-binding-frame-impl old-frame#)))]
...
(defn take! [state blk c]
  (if-let [cb (impl/take! c (fn-handler
                             (fn [x]
                               (aset-all-internal! state VALUE-IDX x STATE-IDX blk)
                               (run-state-machine-wrapped state))))]
    (do (aset-all-internal! state VALUE-IDX @cb STATE-IDX blk)
        :recur)
    nil))

(defn put! [state blk c val]
  (if-let [cb (impl/put! c val (fn-handler (fn [ret-val]
                                             (aset-all-internal! state VALUE-IDX ret-val STATE-IDX blk)
                                             (run-state-machine-wrapped state))))]
    (do (aset-all-internal! state VALUE-IDX @cb STATE-IDX blk)
        :recur)
    nil))
